...
try:
    ...
    # List of data to get from the database about each diskserver in
    # intevention. For now this is also a list of column headings
    listOfSelectValues = ["machineName", "virtualOrganisation", "diskPool",
                          "dxtx"]
    ...
    # this is a list of dictionaries which have keys defined by the select
    # values above
    ...
    if "diskPool" in listOfSelectValues:
        # if there is a diskpool column then we can add a Space Token column
        # after the diskpool
    ...
    for item in listOfResults:
        ...
        for selectValue in listOfSelectValues:
            try:
                ...
                # if there is a key error then it wasn't returned from the
                # database for the moment this means just use a blank row
                toAppend = ""
                logger.warning(("{0} was not returned from magdb - leaving "
                                "this blank.").format(selectValue))
            else:
                logger.debug("Retrieved {0} successfully from MagDB"
                             .format(selectValue))
            thisRow.append(toAppend)
            if selectValue == "diskPool":
                # if we are in the diskPool column then we need to add a column
                # for space token
                try:
                    ...
                    # diskpool
                    thisRow.append("")
                    logger.info(("No spacetoken for this diskpool: {0} - "
                                 "leaving this blank.").format(toAppend))
                else:
    ...
    # do the HTML version as well. This one formats the machine name with a
    # link which goes through to
    ...
    if "machineName" in listOfSelectValues:
        ...
        for row in jsonObj[0]["rows"]:
            # go through each row and change the value of the machine name col
            # to a html link
