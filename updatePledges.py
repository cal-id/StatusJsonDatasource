...
try:
    ...
    for year in range(2009, datetime.datetime.now().year + 1):
        ...
        for toAppend in d["aaData"]:
            ...
            # in dictionaryOfTimeData. Store the year in it's place as the
            # first entry.
    ...
    for key in dictionaryOfTimeData:  # Step through CPU, Disk, Tape
        ...
        # Each column entries list starts with the year (replacing the key) and
        # is followed by five sets of four values.

        # The four values appear to be something like:
        #    [pledgeValue, comment, % of total, total].
        # We are interested in pledgeValue so we start at index 1 and step
        # through in multiples of 4.

        # Each of the five sets refer to accounting for a different experiment
        # (or the sum over all four). The labels for these sets are defined in
        # the config variable `PLEDGES_COL_DATA_LABELS`.

        # This object is to be populated by the following for loop. It should
        # end up as a list of five objects - one for each experiment and the
        # sum.
        # Each object contains:
        #  - 'target' - a description of the data (experimentName)
        #  - 'datapoints' - a list of data, timestamp pairs for a grafana time
        # plot
        ...
        for dataInAYear in dictionaryOfTimeData[key]:
            ...
            timestamp = int(time.mktime(dt.timetuple())) * 1000  # Cnvrt to ms
            # Start at index 1, step through in multiples of 4 to step through
            # the data for each experiment in this year.
