...
vo_list = ["alice","atlas","cms","lhcb","hone","ilc","mice","minos","na62","snoplus","t2k","superb","dirac"] # this is the list of vos to go through

for vo in vo_list:
	jsonObj = [{"columns":[],"rows":[],"type":"table"}] # the object which will eventually be converted into JSON
	jsonObj[0]["columns"]=[{"text": "VO/DiskPool"},{"text": "Disk Used (%)"},{"text": "Disk Used (total)"},{"text":"Disk Free"},{"text": "Disk Total"},{"text": "Tape Used"}] # set the predefined column headings
	...
	justThese  = ["GlueSATotalOnlineSize","GlueSAUsedOnlineSize","GlueSAFreeOnlineSize","GlueSALocalID","GlueSATotalNearlineSize","glueSAusednearlinesize","GlueSAFreeNearlineSize"]
	...
	for row in rows:
		...
		thisRow.append(int(round(int(row[1]['GlueSAUsedOnlineSize'][0])*100/int(row[1]['GlueSATotalOnlineSize'][0]))))
	...
	try:
		with  open("/var/www/html/grafanaJsonDatasources/storageUsage"+vo.capitalize()+"/query","w") as outputFile: # have to do it this way to ensure that the file system gets cleaned up if there is an error or something
