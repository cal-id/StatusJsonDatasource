...
listOfGangliaURLs = [
	{
		"link": (
			"http://ganglia.gridpp.rl.ac.uk/ganglia/graph.php?me=RAL+"
...
for urlDict in listOfGangliaURLs:
	...
	for window, seconds in listOfGangliaWindows:
		...
		if not gangliaObj:
			...
			for current, new in zip(gangliaObj, addition):
				...
				current["datapoints"] += [pair for pair in new["datapoints"]
											if pair[1] < earliestDataTime]
	...
	for timeseries in gangliaObj:
		...
		timeseries["metric_name"] = timeseries["metric_name"].replace(r"\g", "")
